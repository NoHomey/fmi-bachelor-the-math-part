...
\begin{document}
    ...
    Нека \(\V, \; \W\) - Л.П над полето \(\F, \; \varphi \in \mathrm{Hom}(\V, \; \W) , \; n = \mathrm{dim}\V, \; m = \mathrm{dim}\W \\\\
    \n{e} - \text{ базис на } \V \\\\
    \m{f} - \text{ базис на } \W \\\\
    ...
    v = \sum_{i = 1}^n \lambda_i e_i = \sum_{i = 1}^n \mu_i f_i \in \V, \; \lambda_i, \mu_u \in \F, \; \ieqn\\
    (\n{\lambda})^t = T(\n{\mu})^t\)
